...
import pandas as pd
df1 = pd.DataFrame(dict(college=college, RANK=rank, data_science=rank))
# df2 = pd.DataFrame(dict(Counts=cnt_y))
fig = px.bar(df1, x=df1.college, y=df1.data_science, color=df1.RANK)
fig.write_html("data_science.html")
fig.show()